...
public class StudentManagTopDaoImpl extends SelectByAll implements StdSearchDao {
	...
	@Override
	public ArrayList<Codes> selectByAll(Codes code, boolean i) {
		String where = "";
		if (i == true) {
			where = "	where d2.`day` = ? and d.deptname = ? and c.grade = ? and a.attending = ?";
		}
		...
		try (Connection con = JdbcUtil.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			if (i == true) {
				pstmt.setString(1, code.getDayno().getDay());
				pstmt.setString(2, code.getDeptno().getDeptname());
				pstmt.setInt(3, code.getGrade());
				pstmt.setString(4, code.getAtdno().getAttending());
			}
...
//						System.out.println(getCodes(rs));
	...
	/*
	 * private Codes getCodes(ResultSet rs) throws SQLException { int no =
	 * rs.getInt("no"); String name = rs.getString("name"); int birthday =
	 * rs.getInt("birthday"); int social = rs.getInt("social"); Days dayno = new
	 * Days(rs.getInt("dayno")); Department deptno = new
	 * Department(rs.getInt("deptno")); int grade = rs.getInt("grade"); Attendings
	 * atdno = new Attendings(rs.getString("atdno")); Militarys miltno = new
	 * Militarys(rs.getString("miltno")); try { dayno.setDay(rs.getString("day"));
	 * deptno.setDeptname(rs.getString("deptname"));
	 * atdno.setAttending(rs.getString("attending"));
	 * miltno.setMilitary(rs.getString("military")); } catch (SQLException e) {
	 * 
	 * }
	 * 
	 * return new Codes(no, name, birthday, social, dayno, deptno, grade, atdno,
	 * miltno); }
	 */
